...
led_level_t leds_values[BAR_INSTANCE][LEDS_COUNT];
led_level_t leds_values_tmp[BAR_INSTANCE][LEDS_COUNT];
BLINKT_BAR_GRAPH_MODE_t bar_graph_mode[BAR_INSTANCE];
BLINKT_ANIMATION_MODE_t animation_mode;
uint32_t animation_param_a;
uint8_t brightness[BAR_INSTANCE][LEDS_COUNT];
...
void BLINKT_SetBrightness(uint8_t instance, uint8_t bright){
    BLINKT_SetLedBrightness(instance, BLINKT_LED_ALL, bright);
}

void BLINKT_SetLedBrightness(uint8_t instance, uint16_t led_mask, uint8_t bright){
  if(bright <= 31){
    for(int n = 0 ; n < LEDS_COUNT ; n++)
    {
      if( (led_mask & (0x0001 << n)) != 0 )
      {
        brightness[instance][n] = bright;
      }
    }
  }
}
...
void BLINKT_refreshDisplay(uint8_t instance) {
  ...
  if( (clk_port[instance] != 0) && (data_port[instance] != 0))
  {
    ...
    for( n = 0 ; n < LEDS_COUNT ; n++)
    {
      for( i = 0 ; i < (LEDX_START_SIZE) ; i++)
      {
        BIT_1(instance);
      }
      for( i = 0 ; i < (LEDX_BRIGHT_SIZE) ; i++)
      {
        if( (brightness[instance][n] & (0x01 << (LEDX_BRIGHT_SIZE - 1 - i))) != 0 )
        {
          BIT_1(instance);
        }
        else
