...
/**
 * @brief  Set all Led brightness (they are set at max by defaults, but consume a lot)
 * @param  instance: instance of the led bar, up to BAR_INSTANCE-1
 * @param  bright: brightness of the led bar, should be between 0 (off) and 31 (max, can consume 2 Amp with 4 instances) 
 * @retval None
 */
void BLINKT_SetBrightness(uint8_t instance, uint8_t bright);

/**
 * @brief  Set Leds brightness (set at max by defaults, but consume a lot)
 * @param  instance: instance of the led, up to BAR_INSTANCE-1
 * @param  led_mask: Bit mask to select the LEDs to update.
 * @param  bright: brightness of the leds, should be between 0 (off) and 31 (max, can consume 2 Amp with 4 instances) 
 * @retval None
 */
void BLINKT_SetLedBrightness(uint8_t instance, uint16_t led_mask, uint8_t bright);
...
void BLINKT_refreshDisplay(uint8_t instance);
